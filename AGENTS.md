...
Controlador nativo (PyQt5) do mouse **Logitech G403 HERO** para Linux (alvo: Linux Mint): DPI, sensibilidade, polling rate, perfis, macros e auto-clicker. Python ≥ 3.10. O fluxo padrão é o app desktop (`start.sh`); o caminho web legado (`mouse_hub.py` na raiz) está **em processo de descontinuação** (issue #10 condiciona o fim dele à paridade nativa) — não o estenda nem dependa dele.
...
| `mouse_hub.py` (raiz) | App web legado — congelado (fim condicionado à paridade nativa, issue #10) |
| `tests/` | Suíte determinística **sem hardware** + benchmarks (`bench_*`) + smoke de UI |

**Regra de ouro** (declarada no próprio `mouse_hub/__init__.py`): regras de domínio vivem somente no `core`. Lógica de DPI/perfil/sensibilidade na UI, na platform ou em módulo legado é bug de arquitetura.
...
O CI (`.github/workflows/ci.yml`) roda em Python 3.12 com 2 jobs: `test` (compileall + imports + pytest + bench) e `ui_smoke` (Xvfb). Um PR só está pronto quando **ambos** passam — e o estado reportado deve vir das checks reais, nunca presumido.

## Regras para agentes

1. **Hardware não existe no CI.** Tudo que depende do mouse físico (HID++, udev, XTest) deve ser testável via fakes (`tests/fakes.py`). Código novo que toque hardware precisa de caminho fakeável + teste determinístico correspondente.
...
- Permissões de udev para HID++ pertencem à máquina do usuário (ver `docs/udev/`). Em ambiente sem permissão, a descoberta do device deve degradar com elegância (fallback documentado), nunca crashar.
- `python-xlib` e `PyQt5` são dependências de produção (comentário no `pyproject.toml`); não adicione dependências pesadas sem necessidade.
