...
| FR-003 / SC-002: ponte sem escrita e projeto local | AST do parser + inspeção do launcher + `get_current_config` + probe de `replace_content` | **PASS**: nenhum subcomando de edição; launcher usa o venv local e o `--project` aponta para `ROOT`; edição real foi rejeitada como ferramenta inativa e não alterou o SHA da configuração. |
